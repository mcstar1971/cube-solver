...
function detectColor(rgb) {
  ...
  // 橙色：色相 10-50，介于红和黄之间
  // 关键特征：G值比红色高（橙色偏黄）
  if (h >= 10 && h <= 50 && s > 30 && v > 40) {
    // 用G/R比值区分橙/红边界
    if (r > 0 && g / r > 0.35) {
      return 'B'
    }
  }
  // 橙色的RGB特征：R高，G中等（比红色高），B低
  if (r > 180 && g > 80 && g < 200 && b < 100 && g / r > 0.35) {
    return 'B'
  }
  
  // ===== 红色检测 =====
  // 红色：色相 340-360 或 0-15（缩小范围，给橙色留空间）
  // 特点：R高，G和B低
  if ((h >= 340 || h <= 15) && s > 30 && v > 30) {
    return 'F'
  }
  // 红色的RGB特征：R高，G很低
  if (r > 150 && g < 100 && b < 100 && s > 35) {
    return 'F'
  }
